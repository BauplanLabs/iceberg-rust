...
fn scalar_value_to_datum(value: &ScalarValue) -> Option<Datum> {
    match value {
        ...
        ScalarValue::TimestampNanosecond(Some(v), tz) => {
            interpret_timestamptz_nanos(*v, tz.as_deref())
        }
        _ => None,
    }
}
...
fn interpret_timestamptz_nanos(nanos: i64, tz: Option<impl AsRef<str>>) -> Option<Datum> {
    let offset = tz.and_then(|s| s.as_ref().parse::<FixedOffset>().ok());

    match offset {
        Some(off) => Some(Datum::timestamptz_from_datetime(off.timestamp_nanos(nanos))),
        None => Some(Datum::timestamp_nanos(nanos)),
    }
}
